...
const buildChartData = (data, casesType) => {
  if (!data) {
    alert('No graphable data for this country.');
    return false;
  }
  let chartData = [];
  let lastDataPoint;
  for (let date in data.cases) {
    if (lastDataPoint) {
      let newDataPoint = {
        x: date,
        y: data[casesType][date] < 0 ? 0 : data[casesType][date] - lastDataPoint,
      };
  ...
  useEffect(() => {
    const fetchData = async () => {
      if (country === 'worldwide') {
        ...
        await fetch(`https://disease.sh/v3/covid-19/historical/${country}?lastdays=100`)
        .then((response) => {
          return response.json();
        })
        .then((data) => {
